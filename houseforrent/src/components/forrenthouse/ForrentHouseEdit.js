...
const ForrentHouseEdit = ({ id }) => {
    ...
    useEffect(() => {
        const fetchForRentHouse = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/api/forrent-house/24`);
                const data = response.data;
    ...
    const handleSubmit = async (e) => {
       ...
       if (!address || !decription || !rentingprice || !namehouse || !bedroom || !bathroom) {
        ...
        try {
            ...
            const response = await axios.patch('http://localhost:8080/api/forrent-house/edit/24', formForrent);
